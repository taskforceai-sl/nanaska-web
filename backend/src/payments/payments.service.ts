...
export class PaymentsService {
  ...
  /**
   * Builds the signed form payload sent to the IPG checkout page.
   * Signature algorithm (PayHere):
   *   MD5( merchant_id + order_id + amount + currency + MD5(merchant_secret).toUpperCase() ).toUpperCase()
   */
  private buildIpgPayload(
    orderId: string,
    merchantRef: string,
    amountLkr: number,
    ...
    return {
      ...
      return_url: process.env.IPG_RETURN_URL,
      cancel_url: process.env.IPG_CANCEL_URL,
      notify_url: process.env.IPG_NOTIFY_URL,
      order_id: orderId,
      items: `Nanaska CIMA – ${merchantRef}`,
      currency,
      amount,
